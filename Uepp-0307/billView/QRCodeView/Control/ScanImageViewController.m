...
- (void)setScanView
{
    ...
    UIView *topMaskview = [[UIView alloc] initWithFrame:CGRectMake(0,0, VIEW_WIDTH,54)];
    topMaskview.backgroundColor = [[UIColor blackColor]colorWithAlphaComponent:TINTCOLOR_ALPHA];
    [_scanView addSubview:topMaskview];

    //最上部view
    UIView *alphview  = [[UIView alloc]init];
    [self.view addSubview:alphview];
    [alphview mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(_scanView.mas_top).offset(54);
        make.height.equalTo(_scanView);
        make.width.equalTo(_scanView);
    }];
    
    UIView *upView = [[UIView alloc] initWithFrame:CGRectMake(0,0, VIEW_WIDTH,SCANVIEW_EdgeTop)];
//    upView.alpha =TINTCOLOR_ALPHA;
    upView.backgroundColor = [[UIColor blackColor]colorWithAlphaComponent:TINTCOLOR_ALPHA];
    [alphview addSubview:upView];
    UILabel *amtlab = [[UILabel alloc]init];
    amtlab.font=[UIFont systemFontOfSize:30];
    NSString *str2= [NSString stringWithFormat:@"￥%@",self.amt];
    amtlab.textAlignment = NSTextAlignmentCenter;
    [amtlab setText:str2];
    [amtlab sizeToFit];
    amtlab.textColor = [UIColor whiteColor];
      amtlab.frame = CGRectMake(0,0, 200, 30);
    amtlab.center = upView.center;
  
    [upView addSubview: amtlab];
    ...
    [alphview addSubview:_scanCropView];
...
//    [downView addSubview:_lightButton];
    ...
    [alphview addSubview:_QrCodeline];
    ...
    [alphview addSubview:_QrCodeline1];
